...
// POST /api/attempts/[id]/submit — atomic lock + score + persist. Honors the
// Idempotency-Key header: same key + identical payload replays the original
// receipt; a conflicting payload is a 409. Submitted attempts are immutable.
// Practice flips to feedback_released immediately; assessment returns a
// receipt only.
export async function POST(
  ...
  try {
    ...
    const limit = checkRateLimit(`submit:${clientIp(req)}`);
    if (!limit.allowed) {
      return json(
        429,
    ...
    const result = await services.submitAttempt({
      sessionId,
      attemptId: id,
      payload: body['payload'],
      idempotencyKey,
    });
    return json(200, { attemptId: result.attemptId, status: result.status });
  } catch (err) {
