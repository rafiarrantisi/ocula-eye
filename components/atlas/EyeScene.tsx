  ...
  useEffect(()=>{
    ...
    function frame(now:number){
      ...
      if(!prev||s!==prev){
        ...
        if(prev&&!s.detail&&!prev.detail&&(prev.view!==s.view||prev.isolated!==s.isolated)){scene.remove(eye.root);disposeObject(eye.root);eye=buildEye(s.view==='cutaway'&&!s.isolated);scene.add(eye.root);eye.root.visible=true;}
        ...
        const wholeKey=(!s.detail&&!s.isolated)?s.selected:null;
        ...
        if(!prev||s.reset!==prev.reset||(s.angle!==prev.angle&&!s.detail&&!wholeKey)||s.view!==prev.view||s.isolated!==prev.isolated||s.module!==prev.module||(detailChanged&&!s.detail)){
          if(s.detail){
            const p=detailPose(s);
      ...
      labels.current?.querySelectorAll<HTMLButtonElement>('[data-structure]').forEach(button=>{
        const id=button.dataset.structure as StructureId;const part=eye.parts[id];const visible=!s.detail&&s.labels&&basic.includes(id)&&part.visible;
    ...
    return()=>{disposed=true;cancelAnimationFrame(raf);ro.disconnect();controls.dispose();disposeObject(eye.root);disposeObject(corneaDetail.root);disposeObject(angleDetail.root);disposeObject(lensDetail.root);disposeObject(irisCiliaryDetail.root);disposeObject(retinaDetail.root);disposeObject(onhDetail.root);disposeObject(flows);disposeObject(convGroup);disposeObject(rays);disposeObject(glare);renderer.domElement.removeEventListener('pointerdown',pointerDown);renderer.domElement.removeEventListener('pointerup',pointerUp);renderer.domElement.removeEventListener('pointercancel',pointerCancel);renderer.domElement.removeEventListener('keydown',keydown);renderer.domElement.removeEventListener('webglcontextlost',contextLost);renderer.dispose();renderer.domElement.remove();};
  },[]);
